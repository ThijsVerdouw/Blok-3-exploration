...
def CollectDataForModelling(FileName, debug = False):
    ...
    logger.info('The Dataset for the model contains the following columns: \n' + str(df.info()))
...
def PerformPreProcessing(X,y):  
    ...
    logger.info("Pre gridsearch model performance on a simple logistic regression classifier: %.3f" % clf.score(X_test, y_test))
