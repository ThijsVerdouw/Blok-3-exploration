...
def setAFKFlag(row, threshold, rangeEnd):
    if (
        row[settings.MonthsAFKCol] >= threshold
        and row[settings.MonthsAFKCol] < rangeEnd
    ):
        return 1
    else:
        return 0
...
def setMassiveLossFlag(row, threshold):
    if (
        row[settings.retrurningPlayerCol] == 1
    ):  # if they just came back, their bot died. This is pointless as the aim of this column is to predict if someone is going to leave.
        return 0
...
def addRetentionColumns(df):
    ...
    df[settings.threeMonthsAFKCol] = df.apply(
        setAFKFlag, threshold=3, rangeEnd=6, axis=1
    )
    df[settings.SixMonthsAFKCol] = df.apply(
        setAFKFlag, threshold=6, rangeEnd=12, axis=1
    )
    df[settings.TwelveMonthsAFKCol] = df.apply(
        setAFKFlag, threshold=12, rangeEnd=999999, axis=1
    )
...
def GroupActivePlayersBySkill(df):
    ...
    plt = sns.histplot(
        x=settings.monthsPlayedCountCol,
        data=df,
    ...
    graphTitle = "Active players by skill level"
    plt.set(
        title=graphTitle, xlabel="Number of months played", ylabel="Number of players"
    )
    plt.figure.savefig(
...
def RetentionAnalysis(df):
    ...
    sumDf = sumDf.groupby(
        [settings.monthsPlayedCountCol, settings.skillLevelCol]
    ).mean()

    # Makes a nice lookin graph:
    plt = sns.lineplot(
        x=settings.monthsPlayedCountCol,
        y=settings.FinalPlayedMonthCol,
        data=sumDf,
        hue=settings.skillLevelCol,
    )
    plt.yaxis.set_major_formatter(mtick.PercentFormatter(xmax=1.0))
    plt.set(
        title=graphTitle, xlabel="Number of months played", ylabel="Retention rate (%)"
    )
    plt.figure.savefig(
        str(settings.figdir) + "/" + graphTitle + ".jpg", format="jpg", dpi=600
    )
...
def LossByAFKMonth(df, comparisonColumn, quitColumn=settings.FinalPlayedMonthCol):
    df = df[[comparisonColumn, quitColumn]]
    df = df.groupby(comparisonColumn).mean()
    print(df.head(20))


def QuitPercentageByColumn(
    df, comparisonColumn, graphTitle, yname, quitColumn=settings.FinalPlayedMonthCol
):
...
def corrMat(df):
    ...
    plt.figure.savefig(
        str(settings.figdir) + "/" + graphTitle + ".jpg",
        format="jpg",
        dpi=600,
        bbox_inches="tight",
    )
...
def PerformRetentionAnanlyis(FilePath):
    ...
    QuitPercentageByColumn(
        df,
        settings.MassiveLossCol,
        "Retention rate (%) by bot health",
        "Retention rate (%)",
    )
    QuitPercentageByColumn(
        df,
        settings.MassiveLossCol,
        "Stop rate (%) by bot health",
        "Stop rate (%)",
        quitColumn=settings.TMPandPermanentStopCol,
    )
    corrMat(df)

    logger.info(
        "We have had this many cases of a player quitting and coming back: "
        + str(
            -1
            * sum(
                df[settings.TMPandPermanentStopCol] - df[settings.FinalPlayedMonthCol]
            )
        )
    )
